...
function selectSubarray(arr) {
  let divisionArray = arr.map((i, index) => {
    let subArrays = arr.filter((item) => {
      if(item == arr[index]) {
        return
      } else { return item}
    });
    const subSum = subArrays.reduce((a, b) => a + b);
    const subProduct = subArrays.reduce((a, b) => a * b);
    return Math.abs(subProduct/subSum);
  })

  const indexMinValue = [];
  divisionArray.filter((item, index) => {
    if (item === Math.min(...divisionArray)) {
      return indexMinValue.push(index)
    }
  });
  let result = indexMinValue.map((item) => {
